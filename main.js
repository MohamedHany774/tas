...
// strong 
    ...
    healBtn.addEventListener('click', () => {
      const humanValue = parseFloat(humanProgress.style.width);

      // Check if human progress is less than 90% and there are remaining uses
      if (humanValue < 90 && healUses > 0) {
        const humanIncrease = Math.floor(Math.random() * 26); // Random number between 0 and 25

        let newHumanValue = humanValue + humanIncrease;
